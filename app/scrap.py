...
class BilloflandingSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(1,num_cont+1):
            s = []
            hd = ['type','finalpod','shippedto','pcd']
            ...
            if not bol:
                ins_query = """insert into container (cid, type, finalpod, shippedto, pcd, movement) values (%s,%s,%s,%s,%s,%s) on conflict (cid) do update set
